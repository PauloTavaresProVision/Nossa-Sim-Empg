...
/* ================= Parâmetros do modelo (Excel "Simulador VF") =================
   Prémio Anual = Massa Anual × TAXA_SIMPLES × FACTOR_COMERCIAL × (1 − DESCONTO)
   Massa Anual  = salário mensal equivalente × N_SALARIOS                        */
export const TAXA_SIMPLES     = 0.02;
export const FACTOR_COMERCIAL = 1.425;
export const DESCONTO         = 0.15;

export const N_SALARIOS_OPCOES = [12, 13, 13.5, 14];
export const N_SALARIOS_DEFEITO = 13;

/* conversão do salário introduzido para o equivalente MENSAL, por tipo:
   - anual:   valor/12 (com 13 salários a massa inclui o subsídio: v×13/12)
   - semanal: valor×52/12
   - diario:  valor×26 (26 dias de trabalho/mês, semana de 6 dias) — pressuposto
              a validar com a NOSSA; ajustar aqui se indicarem outra convenção  */
export const TIPOS_SALARIO = {
  mensal:  { rotulo: 'Mensal',  paraMensal: (v) => v },
  anual:   { rotulo: 'Anual',   paraMensal: (v) => v / 12 },
  semanal: { rotulo: 'Semanal', paraMensal: (v) => (v * 52) / 12 },
  diario:  { rotulo: 'Diário',  paraMensal: (v) => v * 26 },
};

export const FORMAS_PAGAMENTO = { anual: 'Anual', semestral: 'Semestral', trimestral: 'Trimestral' };
...
/* opcoes: {nSalarios, tipoSalario} — salarios são os valores introduzidos,
   no tipo escolhido; a massa é calculada sobre o equivalente mensal */
export function calcularPremios(salarios, opcoes = {}) {
  const nSalarios = N_SALARIOS_OPCOES.includes(opcoes.nSalarios) ? opcoes.nSalarios : N_SALARIOS_DEFEITO;
  const tipo = TIPOS_SALARIO[opcoes.tipoSalario] ? opcoes.tipoSalario : 'mensal';

  const massaMensal = salarios.reduce((soma, s) => soma + TIPOS_SALARIO[tipo].paraMensal(s), 0);
  const massaAnual = massaMensal * nSalarios;
  const premioAnual = massaAnual * TAXA_SIMPLES * FACTOR_COMERCIAL * (1 - DESCONTO);
  return {
    nSalarios,
    tipoSalario: tipo,
    massaMensal,
...
/* opções da simulação vindas do pedido, saneadas */
export function normalizarOpcoes(corpo) {
  const nSalarios = Number(corpo.nSalarios);
  const tipoSalario = String(corpo.tipoSalario || '').toLowerCase();
  const formaPagamento = String(corpo.formaPagamento || '').toLowerCase();
  let inicio = String(corpo.inicio || '').slice(0, 10);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(inicio) || isNaN(Date.parse(inicio))) inicio = null;
  return {
    nSalarios: N_SALARIOS_OPCOES.includes(nSalarios) ? nSalarios : N_SALARIOS_DEFEITO,
    tipoSalario: TIPOS_SALARIO[tipoSalario] ? tipoSalario : 'mensal',
    formaPagamento: FORMAS_PAGAMENTO[formaPagamento] ? formaPagamento : 'anual',
    inicio,
  };
}
...
export async function gerarPdfBytes({ nome, telefone, empregados, opcoes = {} }) {
  ...
  y -= 4;
  const nTexto = String(calculo.nSalarios).replace('.', ',') + ' Meses/Ano';
  campo(p1, margem, y, 'N.º de salários:', nTexto);
  campo(p1, margem + 190, y, 'Tipo de salários:', TIPOS_SALARIO[calculo.tipoSalario].rotulo);
  if (opcoes.inicio) {
    const [a, m, d] = opcoes.inicio.split('-');
    campo(p1, margem + 360, y, 'Início do seguro:', d + '/' + m + '/' + a);
  }
  y -= 14;
...
export async function gerarPdfCotacao({ nome, telefone, empregados, opcoes }) {
