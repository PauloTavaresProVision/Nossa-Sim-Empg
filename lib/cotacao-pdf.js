...
export async function gerarPdfBytes({ nome, telefone, empregados }) {
  ...
  function cabecalho(pagina) {
    ...
    /* bloco do produto, à direita: ícone + nome + selo "10" */
    const seloR = 12;
    const seloX = largura - margem - seloR;
    const seloY = topo + 6;
    if (icone) {
      const h = 30;
      pagina.drawImage(icone, { x: seloX - 218, y: topo - 8, width: h * (icone.width / icone.height), height: h });
    }
    pagina.drawText('EMPREGADOS DOMÉSTICOS', { x: seloX - 184, y: topo + 2, size: 10.5, font: negrito, color: VERDE });
    const wCot = fonte.widthOfTextAtSize('Cotação', 8);
    pagina.drawText('Cotação', { x: seloX - seloR - 6 - wCot, y: topo - 9, size: 8, font: fonte, color: VERDE });
    pagina.drawCircle({ x: seloX, y: seloY - 6, size: seloR, color: VERDE });
    const w10 = negrito.widthOfTextAtSize('10', 10);
    pagina.drawText('10', { x: seloX - w10 / 2, y: seloY - 9.5, size: 10, font: negrito, color: rgb(1, 1, 1) });
    return topo - 30;
  }
  ...
  function seccao(titulo) {
    y -= 10;
    p1.drawLine({ start: { x: margem, y }, end: { x: largura - margem, y }, thickness: 1, color: VERDE });
    y -= 13;
  ...
  function campo(pagina, x, yy, rotulo, valor) {
    ...
    if (valor) pagina.drawText(' ' + valor, { x: x + wR, y: yy, size: 8, font: fonte, color: TEXTO });
  }
  ...
  y2 -= 10;
  p2.drawLine({ start: { x: margem, y: y2 }, end: { x: largura - margem, y: y2 }, thickness: 1, color: VERDE });
  y2 -= 13;
