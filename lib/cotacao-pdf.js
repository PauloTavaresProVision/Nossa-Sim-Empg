...
/* as fontes standard do PDF só codificam WinAnsi (latim ocidental):
   remove qualquer carácter fora desse alfabeto (emojis, etc.) para o
   texto vindo do utilizador nunca rebentar a geração */
function limparTexto(texto) {
  return String(texto || '').replace(/[^ -~ -ÿ€‘’“”–—…]/g, '').trim();
}
...
export async function gerarPdfBytes({ nome, telefone, empregados }) {
  ...
  const salarios = empregados.map((e) => e.salario);
  ...
  linhaValor('N.º de empregados', String(empregados.length), true);
  empregados.forEach((e, i) => {
    let rotulo = 'Salário mensal';
    const quem = e.nome || 'Empregado(a) ' + (i + 1);
    rotulo += ' (' + quem + (e.funcao ? ', ' + e.funcao : '') + ')';
    linhaValor(rotulo, formatAOA(e.salario) + ' Kz', false);
  });
  linhaValor('Massa salarial mensal', formatAOA(calculo.massaMensal) + ' Kz', true);
  linhaValor('Massa salarial anual (13 salários)', formatAOA(calculo.massaAnual) + ' Kz', true);
  ...
  const premioTexto = formatAOA(calculo.premioAnual) + ' Kz';
  ...
  y -= 16;
  linhaValor('Prémio semestral', formatAOA(calculo.premioSemestral) + ' Kz', true);
  linhaValor('Prémio trimestral', formatAOA(calculo.premioTrimestral) + ' Kz', true);
  ...
  /* nota legal e rodapé */
  y -= 26;
  const nota = 'Esta simulação tem carácter meramente informativo e não constitui proposta contratual. Os valores apresentados poderão ser ajustados na emissão da apólice. Prémios exclusivamente em Kz.';
